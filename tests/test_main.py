...
def test_process_vacancies():
    ...
    parameters = {
        "top_n": 2, "filter_words": ["filter1"], "salary_range": "100-200"
        }
...
@patch(
    "builtins.input", side_effect=["test", "3", "filter1 filter2", "100-200"]
    )
@patch("tests.test_main.mock_hh_api")
@patch("tests.test_main.mock_json_saver")
def test_user_interaction(
    mock_json_saver, mock_hh_api, mock_input, mock_print_vacancies
):
    with patch(
        "main._get_vacancies", return_value=[
            Vacancy("Vacancy 1", "url1", 100, "req1")
            ]
    ):
        user_interaction()
