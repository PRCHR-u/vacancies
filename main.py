...
def _get_vacancies(api: HeadHunterAPI, search_query: str) -> list[Vacancy]:
    ...
    vacancies = Vacancy.cast_to_object_list(hh_vacancies) if hh_vacancies \
        else []
    return vacancies
...
def print_vacancies(vacancies) -> list:
    ...
    for vacancy in vacancies:
        print(
            f"{vacancy.title}, url: {vacancy.url}, salary: {vacancy.salary}, "
...
def _save_data(saver: JSONSaver, data: list):
    ...
    unique_data = [item for item in new_data
...
def _get_user_parameters() -> Dict:
    ...
    Returns:
        ...
        (top_n, filter_words, salary_range).
    """
...
def _process_vacancies(
    vacancies: list[Vacancy], parameters: Dict
) -> list[Vacancy]:
    """
    ...
    filtered_vacancies = filter_vacancies(
        vacancies, parameters["filter_words"]
        )
    ...
    top_vacancies = get_top_vacancies(
        ranged_vacancies, len(ranged_vacancies)
        if len(ranged_vacancies) < parameters["top_n"] else parameters["top_n"]
    )
